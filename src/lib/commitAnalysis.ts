/**
 * Mirrors backend/commitAnalysis.js so the UI can fill metrics when the API
 * omits `analysis` (e.g. older server still running).
 */

import type { RepoAnalysis } from "@/types/repoAnalysis";

export type CommitForAnalysis = {
  message: string;
  date?: string | null;
};
...
function classifyNoise(message: string) {
...
function categoriseCommitStyle(message: string) {
...
function computeQualityScore(commits: CommitForAnalysis[]) {
  ...
  for (const c of commits) {
    ...
    totalLen += fl.length;
    if (classifyNoise(c.message) === "merge") mergeN++;
...
function cleanSnippet(message: string) {
...
function dominantPhase(commitsInBucket: CommitForAnalysis[]) {
  let feat = 0;
  let fix = 0;
  let ref = 0;
...
function buildTimeline(commits: CommitForAnalysis[]) {
  const dated = commits
    .filter((c) => c.date)
  ...
  const start = new Date(dated[0].date as string);
  const end = new Date(dated[dated.length - 1].date as string);
  ...
  const buckets: CommitForAnalysis[][] = Array.from({ length: numBuckets }, () => []);
  ...
  const events: RepoAnalysis["timeline"] = [];
  for (const bucket of buckets) {
    if (!bucket.length) continue;
    const bStart = new Date(bucket[0].date as string);
    const bEnd = new Date(bucket[bucket.length - 1].date as string);
...
type BranchInfo = { count: number; truncated?: boolean };

/** Client-side analysis when server omits `analysis` (branch count defaults to 1). */
export function analyzeCommitsClientSide(
  commits: CommitForAnalysis[],
  branchInfo: BranchInfo = { count: 1, truncated: false }
): RepoAnalysis {
